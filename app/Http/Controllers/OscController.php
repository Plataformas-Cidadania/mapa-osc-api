...
class OscController extends Controller
{
    ...
    public function update($id, Request $request) {
        try {
            ...
            $usuario = Auth::user();
            $this->auditService->auditar('updateOsc', 'OSC', $id, $usuario->id_usuario, $data_old, $osc, $request->ip(), $data_old->id_osc);
    ...
    public function getQuantitativoOscPorSituacaoCadastralPorLocalidade($localidadeId){
        try {
            return response()->json($this->service->getQuantitativoOscPorSituacaoCadastralPorLocalidade($localidadeId), Response::HTTP_OK);
        }
        catch (\Exception $e) {
            return $e->getMessage();
        }
    ...
    public function getResumoTrimestralOscsCadastradas($ano){
        try {
            return response()->json($this->service->getResumoTrimestralOscsCadastradas($ano), Response::HTTP_OK);
        }
        catch (\Exception $e) {
            return $e->getMessage();
        }
